...
thread_yield (void) {
    ...
    old_level = intr_disable ();
    if (curr != idle_thread){
        // curr->status = THREAD_READY; // ready 상태로 변경하면 do schedule 실행 X
        list_insert_ordered(&ready_list, &curr->elem, cmp_priority, NULL); // ready_list에 넣어줌
	}
...
void thread_sleep (int64_t ticks) {
    ...
    if (curr != idle_thread) { // idle의 sleep 요청은 무시
        ...
        thread_block(); // status -> block & call schedule()
...
void thread_awake (int64_t ticks) {
	// sleep_thread가 wake_up_tick 기준 오름차순 정렬이므로 첫번째 elem을 깨움
	// list_pop_front()로 구현하기
	while(get_min_awake_tick() <= ticks){
		if(!list_empty (&sleep_list) ){
			struct thread *wakeup_thread = list_entry (list_front(&sleep_list), struct thread, elem);
...
void update_min_awake_tick () {
	// sleep_list의 첫번째 값으로 업데이트
	if(!list_empty (&sleep_list) ){
    	struct thread *next_to_awake = list_entry(list_front(&sleep_list),struct thread,elem); 
    	min_awake_tick = next_to_awake->wake_up_tick; 
	} else { // sleep_list가 비어있으면 Max 값으로 업데이트
